# Test Video Data Generator — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Generate `create_test_data.py`, a script that produces 5 folders of synthetic survey-style `.mp4` + `.txt` pairs under `test_videos/` for manual pipeline verification.

**Architecture:** A single script defines all transcript content as a Python dict, writes `.txt` files directly, and shells out to ffmpeg to generate colored silent `.mp4` files. The pipeline reuses existing `.txt` files and skips Whisper — so only the ffmpeg generation step matters for the video files. The script is idempotent: video generation is skipped if the `.mp4` already exists.

**Tech Stack:** Python stdlib only (`subprocess`, `pathlib`, `os`). ffmpeg (already required by the main pipeline).

---

### Task 1: Scaffold tests and implement `write_transcript`

**Files:**
- Create: `tests/test_create_test_data.py`
...
Expected: `ImportError` — `create_test_data` doesn't exist yet.

- [ ] **Step 3: Create `create_test_data.py` with `write_transcript` only**
...
- [ ] **Step 4: Run write_transcript tests to verify they pass**

```
pytest tests/test_create_test_data.py -k "write_transcript" -v
```
...
```
git add tests/test_create_test_data.py create_test_data.py
git commit -m "feat: scaffold create_test_data with write_transcript"
```

---

### Task 2: Implement `generate_video`

**Files:**
- Modify: `create_test_data.py`

- [ ] **Step 1: Run the generate_video tests to confirm they currently fail**

```
pytest tests/test_create_test_data.py -k "generate_video" -v
```

Expected: 3 FAIL (stub does nothing).

- [ ] **Step 2: Implement `generate_video`**

Replace the stub in `create_test_data.py`:
...
- [ ] **Step 3: Run all generate_video tests**
...
git commit -m "feat: implement generate_video with ffmpeg lavfi color source"
```

---

### Task 3: Define all BUSINESSES data

**Files:**
- Modify: `create_test_data.py`

This task adds all transcript content as a Python data structure. No test changes needed — the existing `test_create_test_data_calls_generate_video_for_each_respondent` test (which asserts `call_count == 23`) will validate the respondent count once `create_test_data()` is wired up in Task 4.

- [ ] **Step 1: Add the BUSINESSES dict to `create_test_data.py`**

Insert after the imports, before `write_transcript`:
...
- [ ] **Step 2: Verify the data structure by running a quick count**
...
### Task 4: Implement `create_test_data()`, run the script, verify output
...
Expected: 4 FAIL (stub does nothing).

- [ ] **Step 2: Implement `create_test_data()`**
...
Expected output (23 lines, one per respondent):
```
...
- [ ] **Step 6: Run the pipeline on one folder to verify end-to-end**

```
python sizzle.py test_videos/riverside_grocery --prompt what did people say about checkout lines
```

Expected: pipeline prints matched timestamps for sarah_k, mike_t, lynn_b and skips james_r. Saves `riverside_grocery.mp4`.

- [ ] **Step 7: Commit**

```
git add create_test_data.py tests/test_create_test_data.py
git commit -m "feat: complete test data generator with orchestration and 23 survey respondents"
```
...
| Folder | Expected respondents for checkout/food/equipment/room/atmosphere prompts |
|--------|-------------------------------------------------------------------------|
| `riverside_grocery` | sarah_k ✓, mike_t ✓, diana_p ~, james_r ✗, lynn_b ✓ |
| `bella_vista_restaurant` | carlos_m ✓, priya_s ✓, tom_h ~, rachel_w ✓, david_l ✗ |
| `iron_fitness_gym` | alex_j ✓, brittany_f ✓, noah_c ~, elena_r ✗ |
| `lakeview_hotel` | mark_s ✓, jessica_t ✓, kevin_d ~, amy_n ✗ |
| `morning_grounds_cafe` | oliver_p ~, sophia_r ✓, ben_k ~, claire_m ✓, theo_v ✗ |

✓ = clearly relevant (should appear in sizzle reel) · ~ = partial · ✗ = not relevant (should be excluded)
