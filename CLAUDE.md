...
**⚠️ One-off jobs cannot run on a free plan.** Verified 2026-07-30: `POST /services/{id}/jobs` returns *"free tier plans are not supported for jobs"* when the job's resolved plan is free. With no `ENCODER_JOB_PLAN_ID` a job inherits the base service's instance type, so a free `sizzle-encoder` makes every dispatch fail. Set `ENCODER_JOB_PLAN_ID` to a paid plan (per-second billed, so the service itself can stay free) or put `sizzle-encoder` on `starter`. Note Starter is 512 MB / 0.5 CPU and `ENCODER_JOB_WORKERS` defaults to 8 — eight `tiny` models is ~600 MB and would OOM, so a bigger `planId` or fewer workers is needed either way.
