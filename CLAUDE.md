...
2. **Phase 2 — Execute:** Clips extracted **in parallel** using `ThreadPoolExecutor(max_workers=min(4, cpu_count))` — clips run concurrently, waiting on all futures before proceeding. The overlay is drawn in the same encode pass by `extract_clip` (`drawtext`), so it adds no extra work.
...
- **`video_editor.py`** — `extract_clip()`: re-encodes to H.264/AAC with fade-out (required — stream copy produces P/B-frame starts that freeze on playback), and optionally burns the identification overlay (`title_lines`) as a fading top `drawtext` in the same pass. `stitch_clips()`: concat demuxer with `-c copy` (safe because clips are I-frame-aligned). `_title_alpha_expr()`, `get_video_dimensions()`, `parse_timestamp_to_seconds()`.
